...
public class AppleService {
  ...
  private AppleTokenResponse getToken(String code) {
    try {
      ...
      AppleTokenResponse response = WebClient.create(appleProperties.tokenUri())
              .post()
              .header("Content-type", "application/x-www-form-urlencoded;charset=utf-8")
              .bodyValue(params)
              .exchangeToMono(res -> {
                if (res.statusCode().is2xxSuccessful()) {
                  ...
                  return res.bodyToMono(String.class)
                          .flatMap(errorBody -> Mono.error(new RuntimeException("Apple API Error: " + res.statusCode() + ", " + errorBody)));
                }
              })
              .block();

      LogUtils.info("Apple getAccessToken: " + response.getAccessToken());
      return response;
    } catch (Exception e) {
      LogUtils.error("Apple getAccessToken error: " + e.getMessage());
  ...
  private OAuthMember makeOAuthMember(AppleTokenResponse token) {
    try {
      ...
      LogUtils.info("Apple getMemberInfo: " + userId);
      return new OAuthMember(userId, email, GenderType.NONE, SocialType.APPLE);
    } catch (Exception e) {
      LogUtils.error("Apple getMemberInfo error: " + e.getMessage());
      throw new ValidationException(AUTH_OAUTH_GET_MEMBER_FAIL, e);
    }
  }
}
